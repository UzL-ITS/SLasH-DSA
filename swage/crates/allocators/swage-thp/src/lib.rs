...
use swage_core::memory::{construct_memory_tuple_timer, ConsecBlocks, GetConsecPfns, PfnResolver, TimerError};
...
impl THP {
    ...
    fn allocate_2m_aligned(size: Size) -> Result<Memory, std::io::Error> {
        ...
        if log_enabled!(log::Level::Debug) && let Ok(consecs) = (aligned, size.bytes()).consec_pfns() {
...
impl ConsecAllocator for THP {
    ...
    fn alloc_consec_blocks(&mut self, size: swage_core::util::Size) -> Result<swage_core::memory::ConsecBlocks, Self::Error> {
        if size.bytes() == 0 || size.bytes() % ALIGN_SIZE.bytes() != 0 {
            return Err(THPError::SizeError(ALIGN_SIZE))
        }
        let mut blocks: Vec<Memory> = vec![];
        let required_blocks = max([size.bytes()/self.block_size().bytes(), 1]).expect("empty iter");
