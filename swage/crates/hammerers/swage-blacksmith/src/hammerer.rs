...
use swage_core::memory::{
    AggressorPtr, BytePointer, ConsecBlocks, DRAMAddr, LinuxPageMap, LinuxPageMapError,
    MemConfiguration, VirtToPhysResolver,
};
use swage_core::util;
...
impl PatternAddressMapper {
    ...
    pub fn get_hammering_addresses_relocate(
        ...
        block_shift: usize,
        memory: &ConsecBlocks,
    ) -> Result<Vec<AggressorPtr>, LinuxPageMapError> {
...
impl HammeringPattern {
    pub fn load_patterns(json_filename: String) -> Result<Vec<HammeringPattern>, PatternLoadError> {
    ...
    pub fn load_pattern_from_json(
        ...
        patterns
            ...
            .ok_or(PatternLoadError::NotFound(pattern_id))
    }
}
...
impl Blacksmith {
    pub fn new(
        ...
        let flush_buf: *mut u8 = util::alloc_util::mmap(std::ptr::null_mut(), MB(1024).bytes());
        ...
        let hammering_addrs = mapping
            .get_hammering_addresses_relocate(
                &pattern.access_ids,
                mem_config,
                block_shift.0,
                memory,
            )
            .expect("relocate hammering addresses");
...
impl Drop for Blacksmith {
    fn drop(&mut self) {
        unsafe {
            ...
            util::alloc_util::munmap(flush_buf, MB(1024).bytes());
        }
    }
}
