...
from hashlib import shake_256, sha256 as hashlib_sha256, sha512 as hashlib_sha512
...
class WOTSKeyData:
    ...
    chains: list[int]
    pk: bytes  # WOTS+ PK
    adrs: ADRS   # the PK address of this WOTS instance
    ...
    def __repr__(self):
        return f"WOTSKeyData at ADRS {self.adrs} with PK {self.pk.hex()}"
    ...
    def calculate_intermediates(self, params: str, adrs: ADRS, pk_seed: bytes, valid_sig: 'WOTSKeyData'):
        ...
        self.intermediates = []
        if self.valid:
            # the chains are intact. Skip the search.
            self.chains_calculated = self.chains
            for i in range(slh.len):
                sig = self.sig[i*slh.n:(i+1)*slh.n]
                h_adrs.set_chain_address(i)
                _, chain_intermediates = slh.chain(sig, self.chains[i], slh.w - 1 - self.chains[i], pk_seed, h_adrs, True)
                self.intermediates.append(chain_intermediates) # type: ignore
            return
        ...
        # compute intermediates for each chain
        for i in range(slh.len):
            h_adrs.set_chain_address(i)
            ...
            for j in range(slh.w):
                ...
                for chain_count, hash in enumerate(chain_intr):
                    if hash == valid_sig.sig[i*slh.n:(i+1)*slh.n]:
                        ...
                        self.intermediates.append(chain_intr)
                        done = True
                ...
                self.intermediates.append(None)
        ...
        for i in range(slh.len):
            ...
            for chain_count, hash in enumerate(chain_intr):
                if hash == self.sig[i*slh.n:(i+1)*slh.n]:
                    ...
                    #print(f"Signature {self.sig_idx}: Recovered chain {i}. Found preimage {self.sig[i*slh.n:(i+1)*slh.n].hex()} of {hash.hex()} at step {chain_count}")
        assert len(self.chains_calculated) == slh.len
        assert len(self.intermediates) == slh.len, f"intermediates length {len(self.intermediates)} != {slh.len}"
        
    def join(self, other: 'WOTSKeyData', pk_seed, params) -> 'WOTSKeyData':
        #print(f"Joining signatures {self.sig_idx} and {other.sig_idx}")
        slh = SLH_DSA(params)
        n = slh.n
        retval = deepcopy(self)
        assert self.adrs == other.adrs
        retval.valid = None
        retval.sig_idx = None
        #assert retval.pk is None or other.pk is None or retval.pk == other.pk, f'{retval.pk} != {other.pk}'
        assert retval.adrs == other.adrs
        if not retval.chains_calculated:
            raise ValueError("No chains calculated")
        mismatched_chains = False
        for idx, chain in enumerate(retval.chains_calculated):
            if other.chains_calculated[idx] < chain:  # we encountered an improved chain
                #print(f"Found improved preimage for chain {idx}: {other.chains_calculated[idx]} < {chain}")
                # test this is actually a preimage
                adrs = self.adrs.copy()
                adrs.set_type_and_clear(ADRS.WOTS_HASH)
                adrs.set_key_pair_address(self.adrs.get_key_pair_address())
                adrs.set_chain_address(idx)
                adrs.set_hash_address(other.chains_calculated[idx])
                expected = retval.sig[idx*n:(idx+1)*n]
                actual = slh.chain(other.sig[idx*n:(idx+1)*n], other.chains_calculated[idx], chain-other.chains_calculated[idx], pk_seed, adrs)
                #print(f"Expected {chain} for chain #{idx}: {expected.hex()}")
                #print(f"Intermediates from {other.chains_calculated[idx]}: {[i.hex() for i in intermediates]}")
                if expected != actual:
                    print(f"chain {idx} mismatch between {self.sig_idx} and {other.sig_idx}, skipping")
                    mismatched_chains = True
                    continue
        ...
        if not mismatched_chains:
            retval.pk = self.calculate_pk(params, self.adrs, pk_seed) if self.verify(pk_seed, params) else other.calculate_pk(params, other.adrs, pk_seed) if other.verify(pk_seed, params) else None
        assert len(retval.chains_calculated) == slh.len
        assert len(retval.sig) == len(self.sig)
        return retval
            
    def try_sign(self, m: bytes | list[bytes], pk_seed, params) -> bytes | bool:
        slh = get_slh(params)
        ...
        for i in range(slh.len):
            if msg[i] < self.chains_calculated[i]:
                return False
        sig = b''
        chain_adrs = self.adrs.copy()
        chain_adrs.set_type_and_clear(ADRS.WOTS_HASH)
        chain_adrs.set_key_pair_address(self.adrs.get_key_pair_address())
        for i in range(slh.len):
            s = self.sig[i*slh.n:(i+1)*slh.n]
            chain_adrs.set_chain_address(i)
            # print(f"Applying {msg[i] - self.chains_calculated[i]} steps of chain")
            sig += slh.chain(s, self.chains_calculated[i], msg[i] - self.chains_calculated[i], pk_seed, chain_adrs)

        return sig
    
    def verify(self, pk_seed, params) -> bool:
        slh: SLH_DSA = get_slh(params)
        # Invert a base_2b conversion.
        # Given that elsewhere base_2b converts a bytes object into a list of out_len integers,
        # this helper reconstructs the original bytes by concatenating the b-bit chunks.
        def inv_base_2b(digits: list[int], b: int) -> bytes:
            total = 0
            for digit in digits:
                if digit >= (1 << b):
                    raise ValueError("digit out of range")
                total = (total << b) | digit
            num_bytes = (len(digits) * b + 7) // 8
            return total.to_bytes(num_bytes, byteorder="big")

        m = inv_base_2b(self.chains[:slh.len1], slh.lg_w)
        
        csum = slh.csum(m)
        
        sig = deepcopy(self.sig)

        # continue chains for csum
        adrs = self.adrs.copy()
        adrs.set_type_and_clear(ADRS.WOTS_HASH)
        adrs.set_key_pair_address(self.adrs.get_key_pair_address())
        for i in range(slh.len1, slh.len):
            cs = sig[i*slh.n:(i+1)*slh.n]
            adrs.set_chain_address(i)
            sig = sig[:i*slh.n] + slh.chain(cs, self.chains[i], csum[i-slh.len1]-self.chains[i], pk_seed, adrs) + sig[(i+1)*slh.n:]

        adrs = self.adrs.copy()
        adrs.set_type_and_clear(ADRS.WOTS_HASH)
        adrs.set_key_pair_address(self.adrs.get_key_pair_address())
        return slh.wots_pk_from_sig(sig, m, pk_seed, adrs) == self.calculate_pk(params, self.adrs, pk_seed)
...
class SLH_DSA:
    ...
    #   initialize
    def __init__(self,  param='SLH-DSA-SHAKE-128f', trace=False):
    ...
    def sha256(self, x, n=32):
        """Tranc_n(SHA2-256(x))."""
        return hashlib_sha256(x).digest()[0:n]

    def sha512(self, x, n=64):
        """Tranc_n(SHA2-512(x))."""
        return hashlib_sha512(x).digest()[0:n]
    ...
    def wots_pk_from_sig(self, sig, m, pk_seed, adrs: ADRS):
        ...
        if isinstance(m, list):
            print("Interpreting m as list of chains")
            msg = m
        else:
            msg     =   self.base_2b(m, self.lg_w, self.len1)
        for i in range(self.len1):
        ...
        msg     +=  self.base_2b(self.to_byte(csum,
            (self.len2 * self.lg_w + 7) // 8), self.lg_w, self.len2)
        assert len(msg) == self.len, f"{len(msg)} != {self.len}"
        tmp     =   b''
        for i in range(self.len):
            adrs.set_chain_address(i)
        ...
        if self.wots_keys is not None and wots_pk_adrs not in self.wots_keys:
            self.wots_keys[wots_pk_adrs] = set()
        if self.wots_keys is not None:
            wots_key_data = WOTSKeyData(sig=sig, adrs=wots_pk_adrs, chains=msg, pk=pk_sig)
    ...
    def csum(self, m):
        ...
        for i in range(self.len1):
            csum    +=  self.w - 1 - msg[i]
        csum    <<= ((8 - ((self.len2 * self.lg_w) % 8)) % 8)
        return self.base_2b(self.to_byte(csum,
            (self.len2 * self.lg_w + 7) // 8), self.lg_w, self.len2)
    ...
    def slh_sign_internal(self, m, sk, addrnd, param=None, r=None, stop_at=None):
        ...
        if stop_at == 0:
            return (sig, pk_fors, ADRS(), i_leaf, i_tree)
        sig_ht, root, ht_adrs, i_leaf  = self.ht_sign(pk_fors, sk_seed, pk_seed, i_tree, i_leaf, stop_at)
        sig     += sig_ht

        return (sig, root, ht_adrs, i_leaf) if stop_at else sig
    ...
    def slh_sign(self, m, ctx, sk, addrnd=None, r=None, param=None, stop_at=None):
        ...
        sig = self.slh_sign_internal(mp, sk, addrnd, r=r, stop_at=stop_at)
        return sig
